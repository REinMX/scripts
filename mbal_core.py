...
class Config:
    # --- model -------------------------------------------------------------
    mbal_file: str = r"C:\Work\Models\three_tank_model.mbi"
...
def config_from_dict(data: dict[str, Any], *, base: Config | None = None) -> Config:
    ...
    if "volume_model" in data:
        raise ValueError(
            ...
            "and optional p90_stoiip/p10_stoiip"
        )
...
def config_to_dict(cfg: Config) -> dict[str, Any]:
    ...
    for tank in cfg.tanks:
        ...
        if tank.p90_stoiip is not None:
...
def _validate_tank_volumes(cfg: Config) -> None:
    """Validate the direct official/P90/P50/P10 volume contract."""
    if cfg.min_tank_stoiip < 0.0 or not math.isfinite(cfg.min_tank_stoiip):
        raise ValueError("min_tank_stoiip must be finite and >= 0")
    for tank in cfg.tanks:
        official = tank.official_stoiip
        if official is None or not math.isfinite(official) or official <= 0.0:
            raise ValueError(
                f"tank {tank.key}: official_stoiip must be greater than zero"
            )
        has_p90 = tank.p90_stoiip is not None
        has_p10 = tank.p10_stoiip is not None
        if has_p90 != has_p10:
            raise ValueError(
                f"tank {tank.key}: p90_stoiip and p10_stoiip must be given together"
            )
        if has_p90:
            assert tank.p90_stoiip is not None
            assert tank.p10_stoiip is not None
            if not (
                math.isfinite(tank.p90_stoiip)
                and math.isfinite(tank.p10_stoiip)
                and 0.0 < tank.p90_stoiip < official < tank.p10_stoiip
            ):
                raise ValueError(
                    f"tank {tank.key}: require 0 < p90_stoiip < "
                    "official_stoiip < p10_stoiip"
                )
...
def build_sample_table(cfg: Config) -> pd.DataFrame:
    ...
    validate_config(cfg)
    uncertain_tanks = [tank for tank in cfg.tanks if tank.p90_stoiip is not None]
    aquifer_distributions = [
        dist
        for tank in cfg.tanks
        for dist in (tank.aquifer_multiplier, tank.aquifer_volume)
        if dist is not None
    ]
    dimensions = len(uncertain_tanks) + sum(
        _needs_random_dimension(dist) for dist in aquifer_distributions
    )
    unit_samples = unit_hypercube(cfg.n_realizations, dimensions, cfg)
    next_dimension = 0

    def draw_distribution(dist: Distribution) -> np.ndarray:
    ...
    for tank in cfg.tanks:
        ...
        if tank.p90_stoiip is not None:
            unit = unit_samples[:, next_dimension]
            next_dimension += 1
        else:
            unit = None
    ...
    for tank in cfg.tanks:
        ...
        if tank.aquifer_volume is not None:
            data[f"aquifer_volume_{tank.key}"] = draw_distribution(tank.aquifer_volume)
...
def percentiles(
    values: np.ndarray, *, extra: tuple[float, ...] = ()
) -> dict[str, float]:
...
def summarize(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    rows = []
    official_by_column = {
        f"stoiip_{tank.key}": float(tank.official_stoiip)
        for tank in cfg.tanks
        if tank.official_stoiip is not None
    }
    official_by_column["stoiip_total"] = sum(official_by_column.values())
    for column, label in _summary_columns(
        ...
        rows.append(
            {
                "variable": label,
                "official": official_by_column.get(column, float("nan")),
...
def _print_volume_diagnostics(samples: pd.DataFrame, cfg: Config) -> None:
    ...
    if "stoiip_total" in source.columns:
        stats = percentiles(
            source["stoiip_total"].to_numpy(), extra=cfg.extra_percentiles
        )
        rows.append(
            {
                "tank": "field",
...
def main(
    ...
    *,
    ...
    if len(stoiip_columns) > 1:
        print("\nSample STOIIP rank correlation (tank priors are independent):")
