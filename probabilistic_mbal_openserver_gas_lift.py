...
volumes are also configured and sampled independently for each tank. Gas-lift
sensitivity can be run as a deterministic sweep across every probabilistic realization.

For every realization the script:
    1. writes every tank's STOIIP, optional aquifer volume and gas-lift rate into MBAL,
...
Usage
-----
    python probabilistic_mbal_openserver_gas_lift.py --dry-run
    python probabilistic_mbal_openserver_gas_lift.py --n 500
    python probabilistic_mbal_openserver_gas_lift.py --model C:\\Models\\field.mbi
    python probabilistic_mbal_openserver_gas_lift.py --gas-lift-values 0,0.5,1.0,1.5
...
class TankConfig:
    ...
    key: str
    # Exact MBAL tank name used by OpenServer. Replace the placeholder below.
    name: str
    index: int  # retained for prediction-result tags until exact result tags are copied
    stoiip: Distribution
...
def _default_tanks() -> tuple[TankConfig, ...]:
    ...
    return (
        TankConfig(
            ...
            index=0,
            # TODO: replace with your real STOIIP uncertainty.
            stoiip=Distribution(kind="lognormal", p90=20.0, p10=70.0),
            # TODO: set your actual aquifer-volume uncertainty, or leave None.
            aquifer_volume=None,
        ),
        TankConfig(
            key="top",
...
class Config:
    ...
    # Replace with the exact well name copied from your MBAL OpenServer dialog.
    gas_lift_well: str = "REPLACE_WITH_GAS_LIFT_WELL_NAME"
    gas_lift_prediction_index: int = 1
    # Empty = do not touch gas lift. CLI --gas-lift-values overrides this.
    ...
    # --- run control -------------------------------------------------------
    out_csv: str = "mbal_gas_lift_results.csv"
    out_dir: str = "mbal_gas_lift_output"
...
# -----------------------------------------------------------------------------
# Input tags below are copied from your MBAL V16.5 OpenServer dialogs.
# {tank} and {well} are exact MBAL object names; {p} is the prediction-well index.
# Output/command tags are STILL placeholders until copied from this exact model/version.

TAGS = {
    # Model-specific input tag templates; object names are configured above.
...
def apply_realization(srv: SetServer, row: pd.Series, cfg: Config) -> None:
    ...
    for tank in cfg.tanks:
        srv.set(
            TAGS["tank_stoiip"].format(tank=tank.name),
            float(row[f"stoiip_{tank.key}"]),
        )
        if tank.aquifer_volume is not None:
            srv.set(
                TAGS["aquifer_volume"].format(tank=tank.name),
                float(row[f"aquifer_volume_{tank.key}"]),
            )

    if "gas_lift_rate" in row.index:
        srv.set(
            TAGS["gas_lift_rate"].format(
                well=cfg.gas_lift_well, p=cfg.gas_lift_prediction_index
            ),
            float(row["gas_lift_rate"]),
        )
...
def _summarize_gas_lift(df: pd.DataFrame, cfg: Config) -> None:
    ...
    for gas_lift_rate, group in successful.groupby("gas_lift_rate", sort=True):
        stats = percentiles(group["np_total"].to_numpy(dtype=float))
...
def summarize(df: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    gas_lift_sweep = "gas_lift_rate" in df.columns
    if gas_lift_sweep:
        # Geological inputs are repeated once per lift setting. Summarize each
        # base realization once, and leave all prediction metrics to the
    ...
    for column, label in _summary_columns(
        ...
        stats = percentiles(summary_source[column].to_numpy())
        rows.append(
            {
                "variable": label,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.gas_lift_values:
        try:
            ...
            parser.error(f"invalid --gas-lift-values: {error}")
